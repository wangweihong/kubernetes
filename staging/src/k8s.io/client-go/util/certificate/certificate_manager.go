...
// 负责调用CSR接口向apiserver请求证书签名,等待apiserver通过后更新证书等
type Manager interface {
	// Start the API server status sync loop.
	Start() //异步执行一个周期循环的后台进程，一旦证书过期或者证书签名模板改变(如主体名,域名), 则向apiserver发起证书签名申请等待apiserver通过后
	// 生成新的证书并更新到本地文件系统
...
type Config struct {
	...
	ClientFn CSRClientFunc // 证书签名请求客户端接口
	...
	Template *x509.CertificateRequest //证书签名申请模板
	...
	GetTemplate func() *x509.CertificateRequest // 动态证书签名申请模板获取函数。如果不设置，默认采用Template作为模板
	...
	// generated.
	CertificateStore Store //用来更新文件系统上的证书私钥对
...
type Store interface {
	...
	// a new cert/key pair.
	Current() (*tls.Certificate, error) // 当前的tls证书密钥对
	...
	// to Current().
	Update(cert, key []byte) (*tls.Certificate, error) // 更新文件系统上的证书密钥对
}
...
type manager struct {
	getTemplate func() *x509.CertificateRequest // 获取证书签名申请模板。

	// lastRequestLock guards lastRequestCancel and lastRequest
	lastRequestLock   sync.Mutex
	lastRequestCancel context.CancelFunc       // 用于取消最后一次提交的证书签名申请
	lastRequest       *x509.CertificateRequest // 最后一次提交的证书签名申请。

	dynamicTemplate bool // 是否指定了动态证书签名申请模板。这个标志只是告知getTemplate()每次获取的模板都不一样
	signerName      string
	usages          []certificates.KeyUsage // 证书使用场景
	forceRotation   bool

	certStore Store //负责更新文件系统上证书私钥对
...
func NewManager(config *Config) (Manager, error) {
	...
	// 如果配置没有指定动态模板函数, 则设置动态模板获取函数返回统一的模板
	...
	m := manager{
		...
		getTemplate:             getTemplate,               // 签名模板获取函数
		dynamicTemplate:         config.GetTemplate != nil, // 是否启动动态模板
...
// 异步执行一个周期循环的后台进程，一旦证书过期或者证书签名模板改变(如主体名,域名), 则向apiserver发起证书签名申请等待apiserver通过后
// 生成新的证书并更新到本地文件系统
func (m *manager) Start() {
	...
	go wait.Until(func() {
		//根据证书有效期，算出轮换到期。
		// 1. 如果已存在证书和签名申请模板一致时立即发起申请, 2. 计算已存在证书的有效期计算失效时间作为发起请求时间
		...
		if sleepInterval := deadline.Sub(m.now()); sleepInterval > 0 {
			...
			select {
			case <-timer.C:
				// unblock when deadline
			// 如果证书签名模板已经改变(和最后一次证书签名申请不一致)时，不等待轮换时间，用新的模板发起新的签名申请。
		...
		// 尝试最多5次向apiserver申请证书签名，直到审批通过，或拒绝，或超时。
	...
	if m.dynamicTemplate {
		go wait.Until(func() {
			...
			// 如果新的签名申请模板和已签名的证书不匹配，且模板和最后一次证书签名请求的模板也不一致时
...
// 向apiserver申请证书签名，并等待apiserver通过/拒绝签名
func (m *manager) rotateCerts() (bool, error) {
	klog.V(2).Infof("Rotating certificates")

	// 生成私钥，基于证书管理器的证书请求模板生成证书签名请求
	// csrPEM为证书签名请求PEM编码, keyPem为私钥PEM编码
...
func (m *manager) nextRotationDeadline() time.Time {
	...
	// 如果已签名的证书和 申请模板不一致时, 需要立即向apiserver发起新的证书签名申请已更新证书。
...
// 生成ecdsa私钥，基于证书管理器的证书请求模板生成证书签名请求
